...
gp.fit(train_gp, train_y)
train_pred_gp = gp.predict(train_gp)
val_pred_gp = gp.predict(val_gp)
...
for i in range(50):
    model.fit(train_x, train_y - train_pred_gp.reshape((-1, 1)), epochs=1, batch_size=BATCH_SIZE)
    predict_train_nn = model.predict(train_x, verbose=0)
    predict_train = predict_train_nn + train_pred_gp
    train_rates.append(sse(train_y, predict_train))
    
    predict_val_nn = model.predict(val_x, verbose=0)
    predict_val = predict_val_nn + val_pred_gp
